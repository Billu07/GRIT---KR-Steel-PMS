                        ...
                        <tbody className="text-[13px] text-[#1A1A1A]">
                            ...
                            {reportType === "equipment" && filteredData.equipment?.slice((previewPage - 1) * previewItemsPerPage, previewPage * previewItemsPerPage).map((eq: any, index: number) => (
                                <tr key={eq.id} className="border-b border-[#F0EDE6] hover:bg-[#FAFAF8]">
                                    ...
                                    <td className="p-4">{eq.model || '-'}</td><td className="p-4">{eq.serialNumber || '-'}</td><td className="p-4">{eq.unit || '-'}</td><td className="p-4">{eq.quantity || '-'}</td><td className="p-4">{eq.location || '-'}</td>
                                    <td className="p-4">{eq.calibrationTestingDate ? format(new Date(eq.calibrationTestingDate), "dd MMM yyyy") : '-'}</td>
                                    <td className="p-4">{eq.calibrationExpiryDate ? format(new Date(eq.calibrationExpiryDate), "dd MMM yyyy") : '-'}</td>
