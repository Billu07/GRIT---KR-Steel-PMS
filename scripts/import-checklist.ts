...
async function main() {
  ...
  for (let i = 5; i < records.length; i++) {
    ...
    // The checkpoint text contains both English and Bengali. Let's just use the whole string for taskName.
    let checkPoint = row[1]?.trim();
    if (!checkPoint) continue;
    
    // Remove newlines and Bengali text for a cleaner task name if desired, or keep as is.
    // For now, let's keep the first line (English) as taskName and the rest as detail.
    ...
    // Columns: Hourly(2), Daily(3), Weekly(4), 15 Days(5), monthly(6), 3 Month(7), 6 Month(8), Yearly(9)
